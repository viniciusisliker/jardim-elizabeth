-- Visão geral da congregação para Superintendente (somente leitura).

CREATE OR REPLACE FUNCTION public.is_superintendente()
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT EXISTS (
    ...
    WHERE id = auth.uid() AND role = 'superintendente'
  );
$$;

REVOKE ALL ON FUNCTION public.is_superintendente() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.is_superintendente() TO authenticated;
