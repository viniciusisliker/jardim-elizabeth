(function () {
  ...
  async function createVisitFromModal(client, toast) {
    ...
    const payload = { title, visit_date: visitDate || null, notes, is_visible: isVisible, updated_at: new Date().toISOString() };
  ...
  async function deleteVisit(client, toast) {
    ...
    const confirmed = window.JEDialog?.confirm
      ...
      : window.confirm('Excluir esta visita e todos os documentos?');
    if (!confirmed) return;
  ...
  async function uploadDocument(client, toast, file) {
    ...
    if (!file || file.size > MAX_BYTES) {
      showToast(toast, file ? 'Arquivo muito grande (máx. 10 MB).' : '', true);
      return;
    }
    const ext = file.name.includes('.') ? file.name.split('.').pop() : 'bin';
    const path = `${currentVisitId}/${Date.now()}_${Math.random().toString(36).slice(2, 8)}.${ext}`;
    const { error: upErr } = await client.storage.from(BUCKET).upload(path, file, { upsert: false, contentType: file.type || undefined });
    if (upErr) {
  ...
  async function deleteDocument(client, toast, docId) {
    ...
    if (!doc) return;
    const confirmed = window.JEDialog?.confirm
      ? await window.JEDialog.confirm({ title: 'Excluir documento', message: `Excluir "${doc.file_name}"?`, confirmLabel: 'Excluir', danger: true })
      : window.confirm(`Excluir "${doc.file_name}"?`);
    if (!confirmed) return;
  ...
  async function initVisit(client) {
    const toast = toastEl();
    if (!window.__JEAdminSecretarioVisitInit) {
      window.__JEAdminSecretarioVisitInit = true;
  ...
  window.JEAdminSecretarioVisit = { initVisit };
})();
